...
# assets/logo-gruene-placeholder.svg
cat > "$root/assets/logo-gruene-placeholder.svg" <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<svg width="180" height="48" viewBox="0 0 180 48" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Logo Platzhalter">
  <defs>
    <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="#84B414"/>
      <stop offset="1" stop-color="#5a8f00"/>
    </linearGradient>
  </defs>
  <rect x="1" y="1" width="178" height="46" rx="10" fill="url(#g)" stroke="#ffffff" stroke-width="2"/>
  <g fill="#ffffff">
    <circle cx="24" cy="24" r="10" opacity="0.85"/>
    <rect x="44" y="16" width="110" height="16" rx="8" opacity="0.9"/>
  </g>
</svg>
EOF
...
<head>
  ...
  <style>
    :root{--brand:#84B414;--brand-ink:#ffffff;--brand-dark:#5a8f00;--bg:#f6f8f5;--paper:#ffffff;--ink:#1a1a1a;--muted:#5d5d5d;--border:#e0e4da;--focus:#0b5fff;--ok:#2e7d32;--warn:#c26500;--danger:#b00020;--radius:12px;--shadow:0 1px 2px rgba(0,0,0,.05),0 8px 24px rgba(0,0,0,.08);--col-gap:1.25rem;--card-gap:1rem;--maxw:1120px;--header-h:72px;--agenda-date-w:330px}
    *{box-sizing:border-box} html,body{height:100%}
    ...
    /* LIVE-BOX */
    .live-box{background:#fff;border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow);padding:.9rem}
    .live-box h3{margin:.1rem 0 .4rem 0;font-size:1.05rem}
    .live-list{display:grid;gap:.5rem}
    .live-item{display:grid;gap:.35rem}
    .live-item .stamp{color:#374;font-size:.9rem}
    .live-item .proof{color:#1f3d1f}
    ...
    .card{break-inside:avoid;background:var(--paper);border:1px solid var(--border);border-radius:var(--radius);margin:0;box-shadow:var(--shadow);overflow:hidden;width:100%;display:inline-block;opacity:1;transform:translateY(0) scale(1);transition:opacity .24s ease, transform .24s ease}
    .card.enter{opacity:0;transform:translateY(10px) scale(.98)}
    .card.enter-active{opacity:1;transform:translateY(0) scale(1)}
    .card.fade-out{opacity:0;transform:translateY(10px) scale(.98)}
    ...
    .label.bereich{background:#f1f7ea;border-color:#e0e4da;color:#333}
    ...
    .to-top{
      ...
      padding:.6rem .8rem;box-shadow:var(--shadow);font-weight:650;cursor:pointer
    }
    .to-top:focus-visible{box-shadow:0 0 0 3px rgba(11,95,255,.25)}
    @media(min-width:721px){ .to-top{display:none !important} }
  </style>
</head>
<body>
    ...
    <section class="hero" aria-label="Einstieg">
      <div class="hero-grid wrap">
        <div class="hero-copy">
          ...
          <div class="live-box tile" aria-label="Aktuelles Update">
  ...
  <script>
    ...
    const DATA = {
      ...
      transparency: [
        { title:"Protokoll Gemeinderat – Mobilität (PDF)", href:"#"},
        { title:"Abstimmungsverhalten – Energievorlage", href:"#"},
        { title:"Sitzungskalender Kommission Verkehr", href:"#"}
      ],
      cards: [
        { id:"c1", title:"Velorouten in Brugg sicher machen", status:"In Arbeit", theme:"Verkehr", region:"Brugg", date:"2025-09-19",
          media:"assets/julia-grieder-aktion.jpg", alt:"Julia Grieder bei einer Begehung der Veloroute in Brugg",
        ...
        { id:"c2", title:"Schulen entlasten – Zeit für Lernen statt Formulare", status:"beschlossen", theme:"Bildung", region:"Kanton Bern", date:"2025-09-12",
        ...
        { id:"c3", title:"Sichere, durchgängige Velorouten", status:"In Arbeit", theme:"Verkehr", region:"Gemeinde", date:"2025-09-10",
        ...
        { id:"c4", title:"Energie fair: Fernwärme mit Sozialtarif", status:"In Arbeit", theme:"Energie", region:"Gemeinde", date:"2025-09-18",
        ...
        { id:"c5", title:"PV-Pflicht für geeignete Dächer", status:"beschlossen", theme:"Energie", region:"Kanton Bern", date:"2025-08-30",
        ...
        { id:"c6", title:"ÖV verlässlicher machen", status:"Entwurf", theme:"Verkehr", region:"Region Emmental", date:"2025-09-05",
        ...
        { id:"c7", title:"Digitale Teilhabe an Schulen", status:"Entwurf", theme:"Bildung", region:"Kanton Bern", date:"2025-09-02",
    ...
    const state = { q:"", themes:new Set(), status:new Set(), themeOrder:[] };
    ...
    function chip(label, group){
      ...
      c.addEventListener("click",()=>{
        ...
        if(pressed){
          ...
          if(group==="themes"){
            state.themeOrder = [label, ...state.themeOrder.filter(t => t!==label)];
          }
        }
        render();
      });
    ...
    function formatCHDate(dateStr,timeStr){const d=parseLocalDateTime(dateStr,timeStr);const fmt=new Intl.DateTimeFormat('de-CH',{weekday:'long',day:'2-digit',month:'long',year:'numeric'});const [hh,mm]=(timeStr||'09:00').split(':');return `${fmt.format(d)}, ${hh}.${mm} Uhr`}
    ...
    function renderAgenda(){
      ...
      const items=(DATA.agenda||[]).slice(0,1);
    ...
    function card(c){
      ...
      const body=el("div",{class:"body"},[el("h3",{},c.title),meta,el("p",{},c.text),el("p",{class:"next"},"Nächster Schritt: "+c.next)]);
      ...
      const article = el("article",{class:"card enter",role:"article","aria-label":c.title, id:c.id},[media,body,actions]);
      requestAnimationFrame(()=>article.classList.add("enter-active"));
      return article;
    }
    ...
    function animateReorder(container, before, after){
      if(prefersReduced) return;
      container.querySelectorAll('.card').forEach(el=>{
        const id=el.id;
        if(before[id] && after[id]){
          const dx = before[id].left - after[id].left;
          const dy = before[id].top  - after[id].top;
          if(dx || dy){
    ...
    function render(){
      ...
      const before = getPositions(host);
      const list = DATA.cards.filter(matchesFilters).sort(sortCards);
      host.innerHTML="";
      if(list.length===0){
      ...
      const after = getPositions(host);
      animateReorder(host, before, after);
      host.setAttribute("aria-busy","false");
    }
    ...
    function setAbout(visible){
      ...
      if(visible){
        history.replaceState(null,"","#ueber-mich");
    ...
    function renderTransparency(){
      ...
      links.forEach(d=>{const t=d.title||d.href||"Link"; ul.appendChild(el("li",{},el("a",{href:d.href||"#"},t)))});}
    ...
    (function initTopButton(){
      ...
      const prefersReduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      function toggle(){
        if (window.innerWidth <= 720 && window.scrollY > window.innerHeight){
  ...
  <button id="toTop" class="to-top" aria-label="Nach oben">Nach oben</button>
</body>
</html>
EOF
